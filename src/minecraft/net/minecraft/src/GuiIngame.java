...
public class GuiIngame extends Gui
{
    ...
    public void renderGameOverlay(float f, boolean flag, int i, int j)
    {
        ...
        if(!mc.gameSettings.showDebugInfo) {
            ...
            Calendar cal = Calendar.getInstance();
            String time = sdf.format(cal.getTime());
        	fontrenderer.drawStringWithShadow( Resiliant.client_name , 1, 1 ,Rainbow.getRainbow(19990, -60998980));
        	fontrenderer.drawStringWithShadow(" (Rel- " + Resiliant.client_version + ") ", fontrenderer.getStringWidth(Resiliant.getClient_name()), 0 , 0xffffffff);
        	fontrenderer.drawStringWithShadow(time, 1, 19, 0xffffffff);
        	
        	//fontrenderer.drawStringWithShadow(packet.readPacket(datainputstream, flag1), 1, 59, 0xffffffff);
        	//" (Rel- " + Resiliant.client_version + ")
        	fontrenderer.drawStringWithShadow( "Valuables ", 1, 9, 0xffecf0f1);
        	drawString(fontrenderer, (new StringBuilder()).append("X: ").append((int)mc.thePlayer.posX).toString(), 2, 29, 0xffecf0f1);
        	drawString(fontrenderer, (new StringBuilder()).append("Y: ").append((int)mc.thePlayer.posY).toString(), 2, 39, 0xffecf0f1);
        	drawString(fontrenderer, (new StringBuilder()).append("Z: ").append((int)mc.thePlayer.posZ).toString(), 2, 49, 0xffecf0f1);

			//fontrenderer.drawStringWithShadow("Sneak Enabled", k - fontrenderer.getStringWidth("Sneak Enabled") , 2, 0xffff0000);
			
        	int yCount = 2;
        	for(Module m : Resiliant.modulemanager.activeModules) {
        		m.onRender();
        		if(m.getState() && !m.getCategory().equals(Category.GUI)) {
        			fontrenderer.drawStringWithShadow(m.getName(), k - fontrenderer.getStringWidth(m.getName()), yCount, m.getColor());
        			yCount += 10;
        		}
