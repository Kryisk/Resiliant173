...
//    	this.setEntityBrightness(10F);
    ...
    {
        return worldObj.handleMaterialAcceleration(boundingBox.expand(0.0D, -0.40000000596046448D, 0.0D).func_28195_e(0.001D, 0.001D, 0.001D), Material.water, this);
    }
            ...
            float f1, int i)
    {
        setPosition(d, d1, d2);
        setRotation(f, f1);
        List list = worldObj.getCollidingBoundingBoxes(this, boundingBox.func_28195_e(0.03125D, 0.0D, 0.03125D));
        if(list.size() > 0)
        {
            double d3 = 0.0D;
            for(int j = 0; j < list.size(); j++)
